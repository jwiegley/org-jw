...
loc1 :: Loc
loc1 = force (Loc "from-file.org" 42)
  ...
  testGroup
    ...
    [ testGroup
        ...
        [ testCase "real ID property is found" $
            let e =
                  force
                    baseEntry
                      { _entryProperties = [Property loc0 False "ID" "abc"]
                      }
        ...
        , testCase "real FILE property takes precedence over virtual" $
            let e =
                  force
                    baseEntry
                      { _entryProperties = [Property loc0 False "FILE" "real"]
                      }
    ...
    , testGroup
        ...
        [ testCase "FILE returns entry loc file" $
            force baseEntry ^? (anyProperty $! cfg) "FILE"
              @?= Just "from-file.org"
        , testCase "CATEGORY returns entry loc file" $
            force baseEntry ^? (anyProperty $! cfg) "CATEGORY"
              @?= Just "from-file.org"
        , testCase "ITEM returns entry headline" $
            force baseEntry ^? (anyProperty $! cfg) "ITEM"
              @?= Just "Top task"
        , testCase "TITLE returns entry title" $
            force baseEntry ^? (anyProperty $! cfg) "TITLE"
              @?= Just "the title"
        , testCase "OFFSET returns entry loc pos as shown" $
            force baseEntry ^? (anyProperty $! cfg) "OFFSET" @?= Just "42"
        , testCase "DEPTH returns entry depth as shown" $
            force baseEntry ^? (anyProperty $! cfg) "DEPTH" @?= Just "3"
        , testCase "PRIORITY returns priority when present" $
            force (baseEntry{_entryPriority = Just "A"})
              ^? (anyProperty $! cfg) "PRIORITY"
              @?= Just "A"
        , testCase "PRIORITY returns Nothing when absent" $
            force baseEntry ^? (anyProperty $! cfg) "PRIORITY" @?= Nothing
        , testCase "CONTEXT returns context when present" $
            force (baseEntry{_entryContext = Just "home"})
              ^? (anyProperty $! cfg) "CONTEXT"
              @?= Just "home"
        , testCase "CONTEXT returns Nothing when absent" $
            force baseEntry ^? (anyProperty $! cfg) "CONTEXT" @?= Nothing
        , testCase "VERB returns verb when present" $
            force (baseEntry{_entryVerb = Just "Read"})
              ^? (anyProperty $! cfg) "VERB"
              @?= Just "Read"
        , testCase "LOCATOR returns locator when present" $
            force (baseEntry{_entryLocator = Just "p42"})
              ^? (anyProperty $! cfg) "LOCATOR"
              @?= Just "p42"
        ...
        , testCase "KEYWORD returns Nothing without keyword" $
            force baseEntry ^? (anyProperty $! cfg) "KEYWORD" @?= Nothing
        ...
        , testCase "TAGS returns serialized tag string" $
            let e =
                  force
                    baseEntry
                      { _entryTags = [PlainTag "foo", PlainTag "bar"]
                      }
             in e ^? (anyProperty $! cfg) "TAGS" @?= Just "::foo:bar::"
        , testCase "TAGS on empty tags" $
            force baseEntry ^? (anyProperty $! cfg) "TAGS" @?= Just ":::"
        ]
    , testGroup
        ...
        , testCase "DEADLINE returns Time-serialized string" $
            let e =
                  force
                    baseEntry
                      { _entryStamps =
                          [DeadlineStamp loc0 (force (mkTime 60000))]
                      }
    ...
    , testGroup
        ...
        [ testCase "unknown property yields empty fold" $
            force baseEntry ^? (anyProperty $! cfg) "NOT-A-THING"
              @?= Nothing
        , testCase "unknown property produces empty list" $
            toListOf ((anyProperty $! cfg) "DOES-NOT-EXIST") (force baseEntry)
              @?= ([] :: [String])
        ]
    , testGroup
