  ...
  testGroup
    ...
    , testCase "showOrgFile with one entry containing body paragraph" $
        let e =
            ...
            hdr = Header [] [] (Body [])
            out = showOrgFile cfg (OrgFile "x.org" hdr [e])
         in out
              @?= [ "* hello"
                  , "line1"
                  , "line2"
    ...
    , testCase "showEntry minimum spacer when tags don't fit" $
        ...
        let smallCfg = cfg{_tagsColumn = 1}
            ...
            hdr = Header [] [] (Body [])
            out = showOrgFile smallCfg (OrgFile "x.org" hdr [e])
        ...
        let e = entry{_entryTitle = "[[http://x][label]]"}
         ...
         in case out of
              (first : _) ->
                assertBool
                  (show first)
                  ( "[[http://x][label]]" `elem` words first
                      || "* [[http://x][label]]" == first
                  )
              [] -> assertFailure "no output"
    , testCase "showEntry link title with tags triggers regex-aware prefixLength" $
        ...
        let e =
            ...
            hdr = Header [] [] (Body [])
            out =
              map (\s -> length s `seq` s) (showOrgFile cfg (OrgFile "x.org" hdr [e]))
         in assertBool
              (show out)
    ...
    , testCase "child entries render recursively with increased depth" $
        let child = entry{_entryDepth = 2, _entryTitle = "child"}
            ...
            hdr = Header [] [] (Body [])
            out = showOrgFile cfg (OrgFile "x.org" hdr [parent])
         in out @?= ["* hello", "** child"]
    ]
